...
4. 如果你需要使用 React, ReactDOM, 或 Framer Motion，必须通过 CDN (unpkg 或 cdnjs) 以 UMD 格式引入，并使用 Babel standalone 编译包含 JSX 的 <script type="text/babel">。如果你使用 Vanilla JS 或 GSAP，同样通过 CDN 引入。
 ...
 */
async function recordComponentVideo(htmlFilePath, outputVideoPath) {
...
async function main() {
  ...
  // 2. 无头录屏
  await recordComponentVideo(outputHtmlPath, outputVideoPath);
